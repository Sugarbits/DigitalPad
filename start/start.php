...
function generateAstro(){
	...
	if(!isset($log->latestTime) || $log->latestTime < date('Y-m-d')){
		foreach($astroArray as $astro){
			preg_match('/<span>(.*?)<em>(.*?)<\/em><\/span>.*?有效日期:(.*?)综合运势(.*?)爱情运势(.*?)工作状况(.*?)理财投资(.*?)健康指数(.*?)商谈指数(.*?)幸运颜色(.*?)幸运数字(.*?)速配星座(.*?)<div class="lotconts">(.*?)<\/div>/isu', curl('http://vip.astro.sina.com.cn/astro/view/'.$astro[0].'/day/', $astro[0]), $matches);
			file_put_contents($astroDir.$astro[0].'.dat', json_encode(array($utf8_chinese_str->gb2312_big5($matches[1]), $utf8_chinese_str->gb2312_big5($matches[2]), $utf8_chinese_str->gb2312_big5(html2txt($matches[3])), substr_count($matches[4], '<img'), substr_count($matches[5], '<img'), substr_count($matches[6], '<img'), substr_count($matches[7], '<img'),$utf8_chinese_str->gb2312_big5(html2txt($matches[8])), $utf8_chinese_str->gb2312_big5(html2txt($matches[9])), $utf8_chinese_str->gb2312_big5(html2txt($matches[10])), $utf8_chinese_str->gb2312_big5(html2txt($matches[11])), $utf8_chinese_str->gb2312_big5(html2txt($matches[12])), $utf8_chinese_str->gb2312_big5($matches[13]))));
		}
		file_put_contents($fileLog, json_encode(array('latestTime' => html2txt($matches[3]))));
		unset($matches);
	}
